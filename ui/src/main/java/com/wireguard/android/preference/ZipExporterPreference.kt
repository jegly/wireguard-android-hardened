...
class ZipExporterPreference(context: Context, attrs: AttributeSet?) : Preference(context, attrs) {
    ...
    override fun getSummary() =
        if (exportedFilePath == null) context.getString(R.string.zip_export_summary)
        else context.getString(R.string.zip_export_success, exportedFilePath)
        ...
        BiometricAuthenticator.authenticate(R.string.biometric_prompt_zip_exporter_title, fragment) {
            when (it) {
                is BiometricAuthenticator.Result.Success -> {
                    // Biometric confirmed — safe to export
                    isEnabled = false
                    exportZip()
                }

                is BiometricAuthenticator.Result.HardwareUnavailableOrDisabled -> {
                    // No biometric hardware — warn user that export will be unprotected
                    // and require explicit confirmation before proceeding.
                    MaterialAlertDialogBuilder(context)
                        .setTitle(R.string.zip_export_title)
                        .setMessage(R.string.zip_export_no_biometric_warning)
                        .setPositiveButton(R.string.zip_export_no_biometric_proceed) { _, _ ->
                            isEnabled = false
                            exportZip()
                        }
